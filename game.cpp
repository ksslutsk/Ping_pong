...
#define realesed(b) (!input->buttons[b].is_down && input->buttons[b].changed)
...
simulate_game(Input* input, float dt)
{
	...
	if (current_gamemode == GM_GAMEPLAY)//game
	{
		...
		if (pressed(BUTTON_ESC)) current_gamemode = GM_ESCMENU;
	}
	else if (current_gamemode == GM_MENU)
	{
		...
		draw_text("PING - PONG!", -65, 20, 2, 0xcccccc);
		...
		if (hot_button == 0)
		{
			...
			draw_text("MULTIPLAYER", 20, -10, 1, 0xaaaaaaa);
		}
		else
		{
			draw_text("SINGLE PLAYER", -80, -10, 1, 0xaaaaaaa);
			draw_text("MULTIPLAYER", 20, -10, 1, 0xff0000);
		}

		
	}
	else if (current_gamemode == GM_ESCMENU)
	{
		if (pressed(BUTTON_UP))
		{
			if (current_choose == 0) current_choose = 2;
			else current_choose-=1;
		}
		if (pressed(BUTTON_DOWN))
		{
			if (current_choose == 2) current_choose = 0;
			else current_choose+=1;
		}
		
		if (current_choose== 0)
		{
			draw_text("MAIN MENU", -30, 20, 1, 0xff0000);
			draw_text("CONTINUE", -28, 0, 1, 0xaaaaaaa);
			draw_text("EXIT", -15, -20, 1, 0xaaaaaaa);
		}
		else if (current_choose==1)
		{
			draw_text("MAIN MENU", -30, 20, 1, 0xaaaaaaa);
			draw_text("CONTINUE", -28, 0, 1, 0xff0000);
			draw_text("EXIT", -15, -20, 1, 0xaaaaaaa);
		}
		else
		{
			draw_text("MAIN MENU", -30, 20, 1, 0xaaaaaaa);
			draw_text("CONTINUE", -28, 0, 1, 0xaaaaaaa);
			draw_text("EXIT", -15, -20, 1, 0xff0000);
		}

		if (pressed(BUTTON_ENTER))
		{
			if (current_choose == 0) current_gamemode = GM_MENU;
			else if (current_choose == 1) current_gamemode = GM_GAMEPLAY;
			else exit(0);
		}
